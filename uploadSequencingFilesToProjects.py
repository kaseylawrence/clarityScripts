...
def send_notification_email(api, published_file_info, projects, send_emails_enabled):
    ...
    if not send_emails_enabled:
        print(f"  INFO: Emails disabled - would have sent to {researcher_email}")
        print(f"  Subject: Clarity DEV: Sequencing Files Available - {project_name}")
        print(f"  Files: {file_count} files in {zip_filename}")
...
def main():
    ...
    if args.send_emails:
        ...
        print("Email notifications: DISABLED (use --send-emails to enable)")
    ...
    if args.send_emails:
        ...
        print(f"Email notifications disabled - {len(published_files)} emails would have been sent")
    print(f"{'='*50}")
    ...
    for zip_info in uploaded_zips:
        ...
        if args.send_emails:
            ...
            print(f"    Email notification: DISABLED")
